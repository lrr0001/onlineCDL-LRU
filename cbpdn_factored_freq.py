...
class CBPDN_FactoredScaledDict(sporco.admm.admm.ADMM):
    ...
    def __init__(self,Q,DR,S,R,W,W1,lmbda,dimN=2,opt=None):
        dimK = None
    ...
    def yinit(self,yshape):
        #print('The code for yinit did indeed run.')
        return self.block_cat(self.fft(self.S),np.zeros(self.cri.Nv + (1,) + (self.cri.K,) + (self.cri.M,)))
    ...
    def reconstruct(self):
        return self.ifft(sporco.linalg.inner(self.Df, self.X, axis=self.cri.axisM)) if opt['ReturnX'] else \
            self.ifft(sporco.linalg.inner(self.Df, self.block_sep1(self.Y), axis=self.cri.axisM))
        pass
