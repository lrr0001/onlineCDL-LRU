...
Df = np.empty(sh.shape[0:2] + D.shape[2:],dtype=np.complex128)
Df = sporco.linalg.fftn(D,sh.shape[0:2],(0,1))
...
Q = sherman_morrison_python_functions.factoredMatrix_chol(Df.reshape((a,b,1,1,noc,nof)),rho=1.0)


Df_internal = Df.reshape(a,b,1,1,noc,nof)

print('inverse test:')
print(Q.inv_check_ls(Df_internal))
...
opt = cbpdn_factoredInv.CBPDN_Factored.Options({'Verbose': True, 'rho': rho, 'AutoRho': {'Enabled': False},
...
b = cbpdn_factoredInv.CBPDN_Factored(Q=Q,DR=Df,S=sh,R=R,W=W,lmbda=lmbda,dimN=2,opt=opt)
